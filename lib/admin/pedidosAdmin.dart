...
import 'package:Tortilleria/Models/Reporte.dart';
...
class PedidosAdmin extends StatefulWidget {
  const PedidosAdmin({Key? key});
  @override
  State<PedidosAdmin> createState() => _PedidosAdminState();
}

class _PedidosAdminState extends State<PedidosAdmin> with SingleTickerProviderStateMixin{
  ...
  /*Stream<Map<String, List<dynamic>>> obtenerPedidosStream(List<DocumentSnapshot> users) {
    var controller = StreamController<Map<String, List<dynamic>>>();
    ...
    users.forEach((user) async {
      ...
      var pedidos = pedidosSnapshot.docs.map((doc) => doc.data()).toList();

      controller.add({userId: pedidos});
    });
    ...
    for (var user in usersSnapshot.docs) {
      ...
      if (pedidosSnapshot.docs.isNotEmpty) {
        ...
        for (var pedido in pedidos) {
          ...
          var fecha = pedido['fecha_entrega'].toDate();
                                    ...
                                    onPressed: () async {
                                      try {
                                        ...
                                        await showDialog(
                                          ...
                                          builder: (BuildContext context) {
                                            return AlertDialog(
                                              title: Text('Error al registrar entrega'),
                                              content: Text('$error'),
                                              actions: <Widget>[
                                                TextButton(
                                                  onPressed: () {
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          Text(
                              ...
                              style: const TextStyle(fontSize: 14)),
                          Text(
                              'Fecha en la que se entrego el pedido: ${pedido['fecha_entrega'].toDate()}',
                              style: const TextStyle(fontSize: 14)),
                          Text('Status: ${pedido['status']}',
                              style: const TextStyle(fontSize: 14)),
                          Text('Monto Total: \$${pedido['monto']} Pesos MXN',
                          ...
                          showDialog(
                            ...
                            builder: (BuildContext context) {
                              return AlertDialog(
                                ...
                                content: Text('¿Estás seguro de que deseas eliminar el pedido cancelado?'),
                                actions: <Widget>[
                                  TextButton(
                                    onPressed: () {
                                      Navigator.of(context).pop();
  ...
  FutureBuilder(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                if (pedidosWidgets.isEmpty) {
                  return Text('Ningún cliente ha realizado pedidos');
                }
  ...
  */
  ...
  Future<void> obtenerStockDisponible(int stock) async {
    try {
      ...
      DocumentSnapshot inventarioDoc = await _firestore
          .collection('inventario')
          .doc('WsuYF06MDnKDQzEqd463')
          .get();
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              return StreamBuilder(
                stream: obtenerPedidosStream(users!),
                builder: (context, pedidosSnapshot) {
                  if (pedidosSnapshot.connectionState == ConnectionState.waiting) {
                    ...
                    for (var user in users!) {
                      ...
                      var pedidos = pedidosSnapshot.data![userId] ?? [];
                                ...
                                ListTile(
                                  ...
                                  subtitle: Container(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: <Widget>[
                                        Text('Recibe: ${pedido['nombre']}'),
                                        Text('Dirección: ${pedido['direccion']}',
                                            style: const TextStyle(fontSize: 14)),
                                        Text('Cantidad: ${pedido['cantidad']} Kg de Tortillas',
                                            style: const TextStyle(fontSize: 14)),
                                        Text('Fecha en la que se realizo el pedido: ${pedido['fecha'].toDate()}',
                                            style: const TextStyle(fontSize: 14)),
                                        Text('Status: ${pedido['status']}',
                                            style: const TextStyle(fontSize: 14)),
                                        Text('Monto Total: \$${pedido['monto']} Pesos MXN',
                    ...
                    if (pedidosWidgets.isEmpty) {
                      return Text('Ningún usuario tiene pedidos');
                    }
